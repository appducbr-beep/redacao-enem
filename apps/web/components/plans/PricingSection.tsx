...
const PLAN_LABELS: Record<'pro-monthly' | 'pro-annual', string> = {
...
export default function PricingSection({ userPlan, isLoggedIn }: Props) {
  ...
  const [pendingPlan, setPendingPlan] = useState<'pro-monthly' | 'pro-annual' | null>(null)
  ...
  const handleCpfConfirm = (cpfCnpj: string) => {
    if (!pendingPlan) return
    const plan = pendingPlan
    setPendingPlan(null)
    setLoadingPlan(plan)
    startTransition(async () => {
      const result = await createProSubscription(plan, cpfCnpj)
      if (result.error) {
  ...
  return (
    <>
      {pendingPlan && (
        <CpfModal
          planLabel={PLAN_LABELS[pendingPlan]}
          isLoading={isLoading}
